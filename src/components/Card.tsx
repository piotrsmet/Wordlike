...
export default function Card({ id, cardLetter, flip, color }: cardProps) {
	...
	switch (color) {
		case IsKeyValid.VALID:
			cardInnnerStyle = cardInnnerStyle.concat('bg-[#A1D6B2] ')
			break
		case IsKeyValid.PARTLY_VALID:
			cardInnnerStyle = cardInnnerStyle.concat('bg-[#F0A04B] ')
			break
	...
	return (
		<div
			className={
				' w-[20%] min-w-[48px] aspect-square m-1 sm:m-1.5 flip-card items-center flex justify-center text-2xl'
			}
		>
			<div className={cardInnnerStyle}>
				...
				<div className={'flip-card-back content-center'}>
					{cardLetter ? cardLetter.toUpperCase() : ''}
				</div>
			</div>
		</div>
